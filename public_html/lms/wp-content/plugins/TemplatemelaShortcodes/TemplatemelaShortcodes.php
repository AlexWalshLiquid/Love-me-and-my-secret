		...
		$output .= '<div class="item'.$class.'"><div class="product-block">';
			$output .= '<div class="single-testimonial">';			
				...
				$output .= '<div class="testimonial-content">';
					$output .= '<div class="testimonial-top">'.substr($contents, 0, 170).'</div>';					
				$output .= '</div>';
...
function shortcode_single_slide($atts, $content = null, $code) {
	...
	$output .= '<div class="title">'.$title.'</div>';
	$output .= '<div class="slider-loading"></div><div class="sc-flexslider">';
...
add_shortcode('tm_slider', 'shortcode_single_slide');
function shortcode_single_slider($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		$output .= '<li>';
		if(!empty($link)):
			$output .= '<a target="_Blank" href="'.$link.'"><span class="banner-image" style="background-image: url('.$image.');"><img src="'.$image.'" alt="" height="'.$height.'" width="'.$width.'" /></span></a>';
		else:
			$output .= '<span class="banner-image" style="background-image: url('.$image.');"><img src="'.$image.'" alt="" /></span>';
		endif;
		$output .= '</li>';
		return $output;
	}
add_shortcode('tm_slide', 'shortcode_single_slider');
/***************** Gallery  ****************/
...
<?php $terms_slug = array();
		...
		$output .=  '<div class="contents">'.templatemela_portfolio_excerpt($excerpt_length).'</div>';
		$output .= '<div class="other-box">';
	$output .= '<div class="links">';
	$output .= '<a href="'.$image.'" title="Click to view Full Image" class="icon mustang-gallery"><i class="fa fa-search"></i></a>';
	$output .= '<a href="'.get_permalink().'" title="Click to view Read More" class="icon"><i class="fa fa-link"></i></a>';							
...
function shortcode_tm_home_category($atts, $content = null) {
   ...
   extract(shortcode_atts(array(
	...
	foreach($category_ids_array as $key1){		
		...
		if($category_ids){		
			$output .= '<li class="main-cat"><a href="'.get_category_link( $category_ids ).'" target="_blank">'.$category_ids->name.'</a><span class="new-custom">new</span>';
			$output .= '<div class="cat-container"><ul class="sub-categories"><div class="title">'.$title.'</div>';
			break;
		}
	}	
	...
	$output .= '</ul><div class ="category-image"><a href="'.get_category_link( $category_ids).'"><img src="'.$image.'" alt=""/></a></div></div></li></ul>';
	$output .= '</div>';
	}
	return $output;
...
{	
   extract(shortcode_atts(array(
	...
	if(!empty($text1) || !empty($text2)):
		$textbox .= '<span class="text1">'.$text1.'</span><span class="text2">'.$text2.'</span>'; 
	endif;
	if(!empty($button_text)):
		$textbox .= '<a class="button" href="'.$button_link.'">'.$button_text.'</a>';
	endif;
	$textbox .= '</div>';
		if(!empty($margin)):
		...
		if($hover == "yes"):
			$output .='<div class="banner '.$classname.' animated " data-animated="'.$animation.'" style="'.$variables.';"><a href="'.$link_url.'" target="_blank"><img  data-animated="fadeInLeft" src="'.$image.'" alt=""/>
	   			<span class="hover_glass"></span></a>'.$textbox.'</div>';	
		else :
		  $output .='<div class="banner '.$classname.' animated " data-animated="'.$animation.'" style="'.$variables.';"><a href="'.$link_url.'" target="_blank"><img  data-animated="fadeInLeft" src="'.$image.'" alt=""/>
	   			</a>'.$textbox.'</div>';	
		endif;
...
function shortcode_blog_posts_container($atts, $content = null, $code) {
	...
	while ( $blog_array->have_posts() ) : $blog_array->the_post();
		...
		$src = mr_image_resize($image, 370, 265, true, 't', false);
		if( empty ( $src ) || $src == 'image_not_specified' ):
			$src = get_template_directory_uri()."/images/megnor/placeholder.png";
			$src = mr_image_resize($src, 370, 265, true, 't', false);			
		endif;
		$shorttitle = substr(get_the_title('','',FALSE),0,50);
			...
			if($comments->total_comments > 0):
          		$output .='<div class="comment-block"><div class="comments-link"><a href="'.get_permalink().'">'.$comments->total_comments.' comment</a> </div> <div class="post_author1">'.$post_author.'&nbsp;&nbsp;</div></div>';
       		else:
          		$output .='<div class="comment-block"><div class="comments-link"><a href="'.get_permalink().'">Leave a comment</a> </div> <div class="post_author1">'.$post_author.'&nbsp;&nbsp;</div></div>';
         	endif;
			$output .= '</div></div></div>';
		$i++;
	endwhile;
...
function shortcode_address($atts, $content = null)
{
	extract(shortcode_atts(array(
		...
		if(!empty($title))
			$output .= '<h1 class="simple-type small-title"><span>'.$title.'</span></h1>';
		if(!empty($description))
...
function shortcode_content_banner($atts, $content = null) {
        extract(shortcode_atts(array(
		 'classname' =>'',
		...
		$tmoption_custom_desc1 = get_option('tmoption_custom_desc1');
		$tmoption_custom_desc2 = get_option('tmoption_custom_desc2');
		$tmoption_custom_desc3 = get_option('tmoption_custom_desc3');
		
		$output = '';
		$output .= '<div class="topbar-banner '.$classname.'"><div class="top-banner-inner container">';
		$output .= '<ul class="top-banner-container">';
			if (!empty($tmoption_custom_text1)):
			$output .= '<li class="content1 content"><div class="content-inner"><div class="cms-icon icon1"></div><div class="cms-title">'.$tmoption_custom_text1.'</div><div class="cms-desc">'.$tmoption_custom_desc1.'</div><div class="shopnow"><a class="button" href="'.$button_link.'">'.$button_text.'</a></div></li>';	
			endif;
			if (!empty($tmoption_custom_text2)):
			$output .= '<li class="content2 content"><div class="content-inner"><div class="cms-icon icon2"></div><div class="cms-title">'.$tmoption_custom_text2.'</div><div class="cms-desc">'.$tmoption_custom_desc2.'</div><div class="shopnow"><a class="button" href="'.$button_link.'">'.$button_text.'</a></div></li>';	
			endif;
			if (!empty($tmoption_custom_text3)):
			$output .= '<li class="content3 content"><div class="content-inner"><div class="cms-icon icon3"></div><div class="cms-title">'.$tmoption_custom_text3.'</div><div class="cms-desc">'.$tmoption_custom_desc3.'</div><div class="shopnow"><a class="button" href="'.$button_link.'">'.$button_text.'</a></div></li>';				
			endif;	
		$output .= '</ul>';	
		$output .= '</div></div>';
		return $output;
...
add_shortcode('tm_content_banner', 'shortcode_content_banner');		  
