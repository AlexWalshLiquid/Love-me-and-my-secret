...
function templatemela_setup() {
	...
	register_nav_menus( array(
		 'primary'   => esc_html__( 'TM Header Navigation', 'shine-fashion' ),
    	 'header-menu'   => esc_html__( 'TM Header Shop Links', 'shine-fashion' ),
    	 'footer-menu'   => esc_html__( 'TM Footer Navigation', 'shine-fashion' ),
	) );
	/*
 ...
 * replace to change 'shine-fashion' to the name of your theme in all
 * template files.
 */
function templatemela_textdomain_setup() {	
	load_theme_textdomain( 'shine-fashion', get_template_directory() . '/languages' );
	$locale = get_locale();
...
function templatemela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'shine-fashion' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'shine-fashion' ),
...
function templatemela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'shine-fashion' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'shine-fashion' );
...
<div class="contributor">
  <div class="contributor-info">
    ...
    <div class="contributor-summary">
      ...
      <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'shine-fashion' ), $post_count ); ?> </a> </div>
    <!-- .contributor-summary -->
  </div>
  <!-- .contributor-info -->
</div>
...
function templatemela_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'shine-fashion' ), max( $paged, $page ) );
	}
	return $title;
}
